...
for (i in 1:length(datasets)) {
  ...
  for (time in 1:3) {
    for (i in 1:2) {
      group = groups[i]
      rating_above_median <- data[[dataset]][[group]][[time]]$rating3 >= median_rating[[dataset]][[group]][[time]]
      rating_below_median <- data[[dataset]][[group]][[time]]$rating3 < median_rating[[dataset]][[group]][[time]]
      performance_above_median <- data[[dataset]][[group]][[time]]$performance >= median_performance[[dataset]][[group]][[time]]
      performance_below_median <- data[[dataset]][[group]][[time]]$performance < median_performance[[dataset]][[group]][[time]]
      sleepiness_above_median <- data[[dataset]][[group]][[time]]$rating1 >= median_sleepiness[[dataset]][[group]][[time]]
      sleepiness_below_median <- data[[dataset]][[group]][[time]]$rating1 < median_sleepiness[[dataset]][[group]][[time]]
  ...
  for (y in 1:2) {
    ...
    for (time in 1:3) {
      ...
      for (i in 1:2) {
        group = groups[i]
        performance <- data[[dataset]][[group]][[time]]$performance
        performance <- performance + rnorm(length(performance),
                                           mean=0,
                                           sd=x_range * scatterplot_scatter)
        
        rating <- data[[dataset]][[group]][[time]][[ydata[y]]]
    ...
    legend(performance_bounds[[dataset]][1],
           ...
           cex=0.8,
           pch=1,
           col=c(colours[['control']], colours[[group]]))
    dev.off()
  }
